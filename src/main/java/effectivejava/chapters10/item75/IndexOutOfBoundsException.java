package effectivejava.chapters10.item75;
 ...
 * exception’s string representation, the result of invoking its toString
 * method. This typically consists of the exception’s class name followed by its
 * detail message. Frequently this is the only information that programmers or
 * site reliability engineers will have when investigating a software failure.
 ...
 * the cause of the failure. In other words, the detail message of an exception
 ...
 * between the bounds. This information tells a lot about the failure. Any or
 * all of the three values could be wrong. The index could be one less than the
 * lower bound or equal to the upper bound (a “fencepost error”), or it could be
 * a wild value, far too low or high. The lower bound could be greater than the
 * upper bound (a serious internal invariant failure). Each of these situations
 * points to a different problem, and it greatly aids in the diagnosis if you
 * know what sort of error you’re looking for.
 * 
 * One caveat concerns security-sensitive information. Because stack traces may
 * be seen by many people in the process of diagnosing and fixing software
 ...
 * constructor that looks like below:
 * 
