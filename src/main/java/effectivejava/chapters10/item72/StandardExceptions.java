package effectivejava.chapters10.item72;
 ...
 * 49). This is generally the exception to throw when the caller passes in an
 * argument whose value is inappropriate. For example, this would be the
 * exception to throw if the caller passed a negative number in a parameter
 ...
 * or state, but other exceptions are standardly used for certain kinds of
 * illegal arguments and states. If a caller passes null in some parameter for
 ...
 * Similarly, if a caller passes an out-of-range value in a parameter
 * representing an index into a sequence, IndexOutOfBoundsException should be
 * thrown rather than IllegalArgumentException.
 * 
 * Another reusable exception is ConcurrentModificationException. This exception
 * is at best a hint because it is impossible to reliably detect concurrent
 * modification.A last standard exception of note is
 * UnsupportedOperationException. This exception is used by classes that fail to
 ...
 * may throw. This table summarizes the most commonly reused exceptions:
 * 
 * -----------------------------------------------------------------------------
 ...
 * reuse ArithmeticException and NumberFormatException if you were implementing
 * arithmetic objects such as complex numbers or rational numbers. If an
 * exception fits your needs, go ahead and use it, but only if the conditions
 * under which you would throw it are consistent with the exception’s
 ...
 * in the deck, it could be construed as an IllegalArgumentException (the
