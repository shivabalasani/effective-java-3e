 ...
 * calling hashCode on the two objects must produce the same integer result.
 * 
 * - If two objects are unequal according to the equals(Object) method, it is
 * not required that calling hashCode on each of the objects must produce
 * distinct results.
 * 
 * The key provision that is violated when you fail to override hashCode is the
 * second one: equal objects must have equal hash codes. Do not be tempted to
 * exclude significant fields from the hash code computation to improve
 * performance. Don’t provide a detailed specification for the value returned by
