package effectivejava.chapters11.item84;
 ...
 * the number of processors. This leaves the thread scheduler with little
 * choice: it simply runs the runnable threads till they’re no longer runnable.
 * The program’s behavior doesn’t vary too much, even under radically different
 * thread-scheduling policies. Note that the number of runnable threads isn’t
 * the same as the total number of threads, which can be much higher. Threads
 * that are waiting are not runnable.
 * 
 ...
 * (Item 80), this means sizing thread pools appropriately [Goetz06, 8.2] and
 * keeping tasks short, but not too short, or dispatching overhead will harm
 * performance.
 * 
 * Threads should not busy-wait, repeatedly checking a shared object waiting for
 * its state to change. Besides making the program vulnerable to the vagaries of
 * the thread scheduler, busy-waiting greatly increases the load on the
 * processor, reducing the amount of useful work that others can accomplish. As
 * an extreme example of what not to do, consider this perverse reimplementation
 * of CountDownLatch:
 * 
 ...
 * threads that are unnecessarily runnable. Performance and portability are
 * likely to suffer.
 * 
 ...
 * the program by putting in calls to Thread.yield. You may succeed in getting
 * the program to work after a fashion, but it will not be portable. The same
 * yield invocations that improve performance on one JVM implementation might
 * make it worse on a second and have no effect on a third. Thread.yield has no
 * testable semantics. A better course of action is to restructure the
 * application to reduce the number of concurrently runnable threads.
 * 
 ...
 * tweaking a few thread priorities, but it is rarely necessary and is not
 * portable. It is unreasonable to attempt to solve a serious liveness problem
 * by adjusting thread priorities. The problem is likely to return until you
