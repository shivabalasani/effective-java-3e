package effectivejava.chapters11.item80;
 ...
 * 
 * By the time the second edition of this book came out, java.util.concurrent
 * had been added to Java. This package contains an Executor Framework, which is
 * a flexible interface-based task execution facility. Creating a work queue
 * that is better in every way than the one in the first edition of this book
 ...
 * 79, page 319), you can wait for any or all of a collection of tasks to
 ...
 * call a different static factory that creates a different kind of executor
 * service called a thread pool. You can create a thread pool with a fixed or
 * variable number of threads. The java.util.concurrent.Executors class contains
 * static factories that provide most of the executors you’ll ever need. If,
 * however, you want some thing out of the ordinary, you can use the
 * ThreadPoolExecutor class directly. This class lets you configure nearly every
 * aspect of a thread pool’s operation.
 * 
 ...
 * heavily loaded production server! In a cached thread pool, submitted tasks
 * are not queued but immediately handed off to a thread for execution. If no
 * threads are available, a new one is created. If a server is so heavily loaded
 * that all of its CPUs are fully utilized and more tasks arrive, more threads
 * will be created, which will only make matters worse. Therefore, in a heavily
 * loaded production server, you are much better off using
 ...
 * generally refrain from working directly with threads. When you work directly
 * with threads, a Thread serves as both a unit of work and the mechanism for
 * executing it. In the executor framework, the unit of work and the execution
 * mechanism are separate. The key abstraction is the unit of work, which is the
 * task. There are two kinds of tasks: Runnable and its close cousin, Callable
 * (which is like Runnable, except that it returns a value and can throw
 * arbitrary exceptions). The general mechanism for executing tasks is the
 * executor service. If you think in terms of tasks and let an executor service
 * execute them for you, you gain the flexibility to select an appropriate
 * execution policy to meet your needs and to change the policy if your needs
 * change. In essence, the Executor Framework does for execution what the
...
public class PreferExecutorstoThreads {
	...
	// And here is how to tell the executor to terminate gracefully (if you fail to
	// do this, it is likely that your VM will not exit):
	exec.shutdown();
    }
