package effectivejava.chapters11.item82;
 ...
 * a class’s behavior, its users will be forced to make assumptions. If these
 ...
 * counts. In normal operation, Javadoc does not include the synchronized
 * modifier in its output, and with good reason. The presence of the
 * synchronized modifier in a method declaration is an implementation detail,
 * not a part of its API. It does not reliably indicate that a method is
 * thread-safe.
 * 
 ...
 * safety is an all-or-nothing property. In fact, there are several levels of
 * thread safety. To enable safe concurrent use, a class must clearly document
 * what level of thread safety it supports. The following list summarizes levels
 * of thread safety. It is not exhaustive but covers the common cases:
 *
 ...
 * invocation is surrounded by external synchronization. Thread hostility
 ...
 * thread-hostile, it is typically fixed or deprecated. The generateSerialNumber
 * method in Item 78 would be thread-hostile in the absence of internal
 * synchronization, as discussed on page 322.
 * 
 * For example, the documentation for Collections.synchronizedMap says this: It
 * is imperative that the user manually synchronize on the returned map when
 ...
 * 
 * synchronized(m) { // Synchronizing on m, not s! for (K key : s) key.f(); }
 * 
 * Failure to follow this advice may result in non-deterministic behavior.
 * 
 * A client can mount a denial-of-service attack by holding the publicly
 * accessible lock for a prolonged period. This can be done accidentally or
 * intentionally. To prevent this denial-of-service attack, you can use a
 * private lock object instead of using synchronized methods (which imply a
 * publicly accessible lock):see below
 * 
 * Because the private lock object is inaccessible outside the class, it is
 * impossible for clients to interfere with the object’s synchronization. In
 * effect, we are applying the advice of Item 15 by encapsulating the lock
 * object in the object it synchronizes.
 * 
 ...
 * shown above) or a lock from the java.util.concurrent.locks package.
 * 
 * The private lock object idiom can be used only on unconditionally thread-safe
 * classes. Conditionally thread-safe classes can’t use this idiom because they
 * must document which lock their clients are to acquire when performing certain
 * method invocation sequences.
 * 
 * The private lock object idiom is particularly well-suited to classes designed
 * for inheritance (Item 19). If such a class were to use its instances for
 * locking, a subclass could easily and unintentionally interfere with the
 * operation of the base class, or vice versa. By using the same lock for
 * different purposes, the subclass and the base class could end up “stepping on
 * each other’s toes.” This is not just a theoretical problem; it happened with
 * the Thread class [Bloch05, Puzzle 77].
 * 
 ...
 * Conditionally threadsafe classes must document which method invocation
