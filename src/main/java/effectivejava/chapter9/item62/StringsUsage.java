 ...
 * it is often in string form. There is a natural tendency to leave it that way,
 * but this tendency is justified only if the data really is textual in
 * nature.If it’s numeric, it should be translated into the appropriate numeric
 * type, such as int, float, or BigInteger. If it’s the answer to a yes-or-no
 * question, it should be translated into an appropriate enum type or a boolean.
 * More generally, if there’s an appropriate value type, whether primitive or
 * object reference, you should use it; if there isn’t, you should write one.
 * While this advice may seem obvious, it is often violated.
 * 
 * Strings are poor substitutes for enum types. As discussed in Item 34, enums
 * make far better enumerated type constants than strings.
 * 
 ...
 * example, here’s a line of code that comes from a real system—identifier names
 * have been changed to protect the guilty: // Inappropriate use of string as
 * aggregate type
 * 
 ...
 * error-prone. You can’t provide equals, toString, or compareTo methods but are
 ...
 * thread-local variable facility. Such a facility provides variables for which
 * each thread has its own value. The Java libraries have had a thread-local
 * variable facility since release 1.2, but prior to that, programmers had to
 * roll their own. When confronted with the task of designing such a facility
 * many years ago, several people independently came up with the same design, in
 * which client-provided string keys are used to identify each thread-local
 * variable: see Version 1
 * 
 ...
 * (sometimes called a capability): see Version 2
 * 
 * While this solves both of the problems with the string-based API, you can do
 * much better. You don’t really need the static methods anymore. They can
 * instead become instance methods on the key, at which point the key is no
 * longer a key for a thread-local variable: it is a thread-local variable. At
 * this point, the top-level class isn’t doing anything for you anymore, so you
 * might as well get rid of it and rename the nested class to ThreadLocal: see
 * Version 3
 * 
...
public class StringsUsage {
	...
	// Version 1 : Broken - inappropriate use of string as capability!
	...
	/// Version 3
	public final class ThreadLocalImproved<T> {
		public ThreadLocalImproved() {
